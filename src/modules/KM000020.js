...
// choice 모듈(같은 단어 고르기)
const KM000020 = async function ({ canvas, direction, elements, imageButton, choiceElements, callback }) {
    ...
    if (imageButton) {
        ...
        arrImageButton.forEach((imageButton, index) => {
            let voice;
            if (imageButton && imageButton.textInfo) voice = loadSound(imageButton.soundInfo.url);
            const imgBtnG = imgSetG.g();
            ...
            onRemove: function () {
                if (bool) {
                    ...
                    if (ansCount >= answer.length) {
                        ...
                        fbEl.forEach(el => {
                            console.log(el);
                            el.boxG.attr('opacity', 0);
                            el.hiddenEl.attr('opacity', 1);
                        });
    ...
    if (answer.length === 0) answer = autoAnswer;
};

export default KM000020;
