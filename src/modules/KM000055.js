...
const KM000055 = async function ({
        ...
        if (question.stepElements) {
            question.stepElements.forEach((stepEl, i) => {
                ...
                stepEl.forEach(el => {
                    ...
                    if (el.isQ && isfirst === 0){ 
                        questionBox.push(
                            elementG.data('beforeClass', 's' + (el.meta.shapeStrokeClassNum || 'no'))
                                    .data('focusClass', 's' + (el.focusClassNum || '0002'))
                            );
        ...
        if (question.choice) {
            question.choice.forEach((arr, j) => {
                ...
                choiceStep[j] = new stepChoice({
                    ...
                    btnRxy: rXY,
                    howlerStop: false,
                    // isLastChoiceGroup: (j === question.choice.length - 1), // 210910 마지막 문제 푼 후 터치 안한 엘리먼트는 비활성 안시키기
                    // standardIndex :10,
                    // opacity : 0.4,
                    choiceCallback: function (idx) {
                        if (questionBox[qCount]) {
                            const { cx, cy } = questionBox[qCount].getBBox();
                            questionBox[qCount].attr('opacity', 0);
                            question.choice[qCount].elements[idx].forEach(e => {
                                createElement({
                                    type: e.type,
    ...
    function start() {
        if (convertVisible) choiceAuto(); // 210909
        readingGroup.start();
    }
    ...
    function choiceAuto() {
        ...
        questionBox[qCount].select('.box')
                            .removeClass(questionBox[qCount].data('beforeClass'))
                            .addClass(questionBox[qCount].data('focusClass'));
    }
