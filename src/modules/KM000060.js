...
const KM000060 = async function ({
    ...
    let ans = [];
    let userAns = [];
    // let count = 0; // 한 초이스의 ans 개수 (이 모듈에선 한 초이스 당 ans 1개)
    ...
    if (question.elements) {
        question.elements.forEach(el => {
            const { type, meta } = el;
            createElement({ type, canvas: firstG, meta });
        });
    }
            ...
            }
            {
                ...
                elements.forEach((el, i) => {
                    ...
                    createElement({
                        type,
                        canvas: choiceEl[j][i],
                        meta: { ...meta, fontSize, classNum, cx, cy, bold }
                    });
                    choiceEl[j][i].data('data', { index: i, click: false }).touchOrClick(actionCallback);
                });
            }
            ans[j] = Array.isArray(ansIndex) ? ansIndex : [ansIndex];
        });
    }
